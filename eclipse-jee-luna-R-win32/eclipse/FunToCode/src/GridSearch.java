...
public class GridSearch {
	...
	public static void main(String[] args) {
		...
		while(t!=0)
		{
			...
			int[][] sub_arr = new int[r][c];
			for(int i =0;i<r;i++)
			{
				str_sub_arr[i] = s.next();
				str_sub_arr[i].replace(" ", "");
				//System.out.println("Convertd: " + str_sub_arr[i]);
			}
			...
			for(int i = 0;i<R;i++)
			{
				...
				if(str_arr[i].contains(str_sub_arr[0]))
				{
					...
					while(temp.contains(str_sub_arr[0]))
					{
						...
						pos1 = pos1 + temp.indexOf(str_sub_arr[0]) + 1;
					...
					//str_arr[i] = str_arr[i].substring(pos);
					//if(str_arr[i].contains(str_sub_arr[0])) i--;
					//str_arr[i] = str_arr[i].substring(pos+1); i--;
				}
